function opponentNoteHit(id, direction, noteType, isSustainNote)
	if _G['dadGhostData.strumTime'] == getPropertyFromGroup('notes') and not isSustainNote and curBeat >= 104 then
		createGhost('dad')
	end
	if not isSustainNote then
		_G['dadGhostData.strumTime'] = getPropertyFromGroup('notes')
		updateGData('dad')	
	end
end
...
function createGhost(char)
	...
	setProperty(char..'Ghost'..songPos..'.alpha', 0.5)
